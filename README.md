...
| **Search API** | Поиск по ключевому слову, регистр, отсутствие результатов, `limit`, `offset`, `hasImages`, `isHighlight`, фильтрация по отделу, диапазон дат, порядок query-параметров, превышение `limit` |
...
В ходе тестирования была обнаружена проблема:

- `hasImages=true` — API может возвращать произведения без изображений;

Случай зафиксирован отдельным тестом и отмечен как `xfail`, поскольку проблема находится на стороне API.
